...
$(function () {
	...
	var currentUrl = window.location.origin + location.pathname;
	var currentBaseUrl = currentUrl.substring(0, currentUrl.lastIndexOf('/'));

	function addActiveClass(element) {
		var elementHref = element.attr('href');

		// Normalize elementHref to be a complete URL
		if (elementHref.startsWith('http')) {
			var elementUrl = elementHref;
		} else {
			var elementUrl = new URL(elementHref, window.location.origin).href;
		}

		var elementBaseUrl = elementUrl.substring(0, elementUrl.lastIndexOf('/'));

		if (currentBaseUrl === elementBaseUrl) {
